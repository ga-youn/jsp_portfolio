...
public class Add_paymentList implements Action {

	@Override
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		System.out.println("Add_paymentList.java open");
		...
		if(loginUserInfo == null){//로그인이 안된 경우
			...
			cControl.deletePaymentList(loginUserInfo.getId());
			request.removeAttribute("paymentList");
			
			//객체에 담아 주기
			//cartList.setCseq(cseq);
			paymentList.setPseq(pseq);
			paymentList.setName(product.getName());
			paymentList.setContent(product.getContent());
			paymentList.setCount(optionSnoInput);
			paymentList.setImage(product.getImage());
			paymentList.setPrice(product.getPrice());
			paymentList.setId(loginUserInfo.getId());
			
			
			
			cControl.insertPaymentList(paymentList);
